...
class SeeDealsProfile extends StatefulWidget {
  final PostProfile model;
  SeeDealsProfile(this.model);
  @override
  _SeeDealsProfileState createState() => _SeeDealsProfileState();
}

class _SeeDealsProfileState extends State<SeeDealsProfile> {
  ...
  Future<void> _seeDeals() async {
    ...
    final response = await http.post(
        DealUrl.seeDeals,
        body: {"post_id": widget.model.post_id});
  ...
  // StringBuffer offers = new StringBuffer("${offer}");
                                    ...
                                    child: Column(
                                      children: <Widget>[
                                        ...
                                        Padding(
                                          ...
                                          child: Row(
                                            children: <Widget>[
                                              // for(var i = offer.indexOf("*"); i != -1; i = offer.indexOf("*", i + 1))
                                              Container(
                                                width: MediaQuery.of(context).size.width*0.8,
                                                child: Text(
                                                  x.post_offer,
                                                  // '* ${offer.substring(2)}',
                                                  // "${offer.substring(0)}\n" + offer[i],
                                                  // "* ${offer.substring(3, offer.indexOf('*'))}",
                                                  style: TextStyle(
                                                    fontSize: 18,
                                                  ),
                                                ),
